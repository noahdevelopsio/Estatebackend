...
import { z } from 'zod'
import type { MaintenanceRequests } from '@/types/db'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let query = supabase.from('MaintenanceRequests').select('*').order('created_at', { ascending: false })
...
export async function GET_SIMULATED(req: NextRequest, userId: string) {
  const supabase = await createClient()
  // Simulate user
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (role !== 'tenant') {
      return NextResponse.json({ success: false, error: 'Access denied: Only tenants can create maintenance requests for their property' }, { status: 403 })
    }
    ...
    // Log activity
    await logActivity(supabase, user.id, 'CREATE', 'MaintenanceRequests', `Created maintenance request: ${title}`)
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const role = await getUserRoleForProperty(supabase, user.id, request.property_id)
    if (role !== 'landlord') {
      return NextResponse.json({ success: false, error: 'Access denied: Only landlords can update maintenance requests' }, { status: 403 })
    }
    ...
    // Log activity
    await logActivity(supabase, user.id, 'UPDATE', 'MaintenanceRequests', `Updated status to ${status} for request ${id}`)

    // Create notification for tenant if resolved
    if (status === 'resolved') {
      const { data: tenant } = await supabase.from('Users').select('id').eq('id', data.tenant_id).single()
      if (tenant) {
