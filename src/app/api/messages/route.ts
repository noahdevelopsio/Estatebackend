...
async function canMessage(supabase: any, senderId: string, receiverId: string): Promise<boolean> {
  ...
  return senderPropertyIds.some((id: string) => receiverPropertyIds.includes(id))
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!canMsg) {
      return NextResponse.json({ success: false, error: 'Access denied: Can only message users in the same property' }, { status: 403 })
    }
    ...
    // Log activity
    await logActivity(supabase, user.id, 'CREATE', 'Messages', `Sent message to ${receiver_id}`)
